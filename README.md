...
En esta práctica se requirió hacer dos programas con la sintaxis de la computadora IAS, la cual es una computadora del año 1952 que era binaria y seguía el concepto de programa almacenado. El primer programa tenía que guardar dos números, uno en la dirección 0 y el otro en la dirección 1; estos dos números tenían que ser sumados y después el resultado se tenía que guardar en la ubicación 2. Esta computadora tenía la particularidad de tener instrucciones de 20 bits: los primeros 8 eran para el código de ejecución y los 12 restantes para la ubicación de la memoria en donde se encuentra guardado el dato. Para poder realizar el programa, tuve que recurrir a una tabla que el maestro proporcionó en su material de apoyo para saber cuál código de ejecución era el que necesitaba utilizar.

Como primer paso, tuve que utilizar un código el cual mueve lo que se le indica en la memoria al acumulador; el elemento que moví fue el de la ubicación 0. Tuve que mover el dato porque, para la operación de suma, se le suma lo que está en la ubicación que le indique a lo que se encuentra en el acumulador; el elemento que sumé a lo del acumulador fue el de la dirección 1. Una vez realizada esa acción, regresé a revisar la tabla de instrucciones, la cual poseía una instrucción que mueve lo del acumulador a la dirección que se le indique (en este caso fue la ubicación número 2), y hasta ahí fue donde se concluyó el primer programa.

El segundo fue bastante más complicado: este segundo programa tuvo que hacer la comparación entre los números almacenados en las ubicaciones 0 y 1 para posteriormente guardar el mayor en la ubicación de memoria 2. Para comenzar, revisé en la tabla si había alguna instrucción que directamente hiciera la función de comparación, pero me di cuenta de que no era así; entonces recordé que para determinar cuál era el mayor simplemente tenía que realizar una resta: si el resultado de la resta era negativo, entonces el número que restó era mayor al que se le aplicó la resta; de lo contrario, al que se le aplicó era el mayor. Después de hacer ese análisis, identifiqué que sí había una instrucción de resta; entonces procedí a aplicar la instrucción para mover el que se encontraba en la dirección 0 al acumulador, pero al momento en que iba a realizar la resta me cuestioné cómo iba a recuperar el dato, ya que al restarse se modificaba. Pero después de una investigación entendí que los valores originales se mantenían en sus ubicaciones originales (las cuales eran 0 y 1) hasta que se sobrescribieran, por lo que ahí fue donde utilicé la instrucción de resta.

Sin embargo, después de eso no supe cómo identificar si era negativo o no para la elección del mayor. Ahí fue cuando tuve que investigar otra vez y me informé de la instrucción JUMP, la cual verifica si lo que se encuentra en el acumulador es no negativo: de ser así continúa, si no, realiza un salto hasta una ubicación de memoria que se le haya indicado. Entonces procedí a aplicar la instrucción JUMP, la cual, como ya había mencionado, verifica si el valor que está en el acumulador es no negativo; si no lo es, le indiqué que realice un salto a la dirección 6 de memoria. Ahí está situado otro salto a la dirección 7, en la cual indiqué el código para pasar lo de la ubicación 0 al acumulador; seguido de eso, en la instrucción siguiente, cargar el valor que está en el acumulador a la ubicación 2. Volviendo a la instrucción JUMP que verifica si es negativo o no: si el resultado que está en el acumulador es negativo, entonces continúa a la siguiente línea y carga lo de la ubicación 1 en el acumulador, y después le indiqué que el valor que se almacena en el acumulador pase a almacenarse en la ubicación 2 para finalizar el segundo código.

Posteriormente a eso, contesté una pregunta la cual cuestionaba el porqué de los 12 bits de dirección, la cual contesté con facilidad, ya que era para encajar en el formato de 20 bits (con 8 de código de operación y 12 de dirección); además de eso, al tener 12 bits de dirección se podía tener hasta 4096 palabras para el direccionamiento.
